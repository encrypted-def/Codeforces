...
///////////////////////////////////////////////
  sf2(n,q);
  rep(i,0,n) sf1(arr[i]);
  fill(lidx,lidx+200001,0x7f7fff);
  fill(ridx,ridx+200001,-1);
  rep(i,0,n){
    lidx[arr[i]] = min(i,lidx[arr[i]]);
    ridx[arr[i]] = max(i,ridx[arr[i]]);    
    cnt[arr[i]]++;
  }
  vector<ti3> col;
  rep(i,1,200001){
    if(ridx[i] == -1) continue;
    col.pb({lidx[i],ridx[i],i});
  }
  sort(all(col));
  int cur_mx_ridx = -1;
  int cur_mx_cnt = 0;
  ...
  rep(i,0,sz(col)){
    ...
    cur_mx_ridx = max(cur_mx_ridx,ri);
    tot += cnt[idx];
    cur_mx_cnt = max(cur_mx_cnt,cnt[idx]);
  }
  ans += (tot-cur_mx_cnt);
  pf1l(ans);
